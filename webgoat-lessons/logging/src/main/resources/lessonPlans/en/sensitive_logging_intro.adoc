...
Be careful with logging personal information. For instance: do not log bank-account details, personal identifiable information to which a user did not consent having it logged. Do not log facts which can establish the identity of the subject being logged.
...
And here the problem often starts: access logs sometimes capture the full URL used for the request. This can include sensitive URL-parameters. Therefore: be careful with what you put in the URL as parameters & let's make sure that you do not log those in an open accessible log.
